...
module('Integration | Component | stripe checkout', function(hooks) {
  ...
  test('it renders', async function(assert) {
    ...
    assert.equal(find('*').textContent.trim(), 'Pay with card', 'renders default button text');

    await render(hbs`{{stripe-checkout label='Pay now'}}`);

    assert.equal(find('*').textContent.trim(), 'Pay now', 'renders passed button label text');
    ...
    assert.equal(find('*').textContent.trim(), 'Pay dude!', 'renders using block-form');
  });

...
module('Unit | Service | stripe', function (hooks) {
  ...
  test('registerComponent() registers the component', function (assert) {
    assert.deepEqual(service._alive, {}, '_alive is empty');
  ...
  test('open() opens Stripe Checkout with correct config options', async function (assert) {
    ...
    const configureCheckoutStub = sinon.stub(
      window.StripeCheckout,
      'configure'
    );
    ...
    assert.ok(
      openCheckoutSpy.calledWith(stripeOptions),
      'opens Stripe checkout with correct config options'
    );
  });
  ...
  test('close() closes Stripe Checkout', function (assert) {
    ...
    assert.ok(
      closeCheckoutSpy.calledOnce,
      'closes Stripe checkout when it is opened'
    );
  });
});
